...
func main() {
	...
	mux.HandleFunc("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `{"status":"ok","tunnels":%d,"time":"%s"}`, reg.Count(), time.Now().UTC().Format(time.RFC3339))
	})
	...
	log.Printf("[xtunnel] Starting on :%s | domain: %s", port, domain)
	if err := srv.ListenAndServe(); err != nil {
		log.Fatalf("[xtunnel] %v", err)
	}
}
...
func dashboardHandler(reg *registry.Registry, domain string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, t := range tunnels {
			rows += fmt.Sprintf(
				"<tr><td>%s</td><td><a href=\"https://%s.%s\" target=\"_blank\">%s.%s</a></td><td>%s</td><td>%d</td><td>%s ago</td></tr>",
				t.Name, t.Name, domain, t.Name, domain,
				t.ClientAddr, t.RequestCount,
				time.Since(t.ConnectedAt).Round(time.Second),
			)
		}
		if len(tunnels) == 0 {
			rows = "<tr><td colspan=\"5\" class=\"empty\">No active tunnels &mdash; run <code>xtunnel 3000</code> to start one.</td></tr>"
		}
...
<style>
  ...
  .stat{display:inline-block;background:#fff;border:1px solid #e5e7eb;border-radius:8px;padding:16px 24px;margin-right:12px;margin-bottom:16px}
...
<body>
<h1>&#9889; Xtunnel</h1>
<p class="subtitle">Active tunnels &mdash; auto-refreshes every 10 seconds</p>
